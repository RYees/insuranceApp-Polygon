...
const MyVehicles = () => {
  const { getMyVehicles, myvehicledata, findyourPaymentId, paymentid } = useContext(VehicleContext);
  const { datetoepoch, PayMonthly } = useContext(InsuranceContext);
  ...
  return (
    <div>
      <h1 className='my-10 text-xl ml-2' onClick={findyourPaymentId}>My  Vehicles</h1>      
      ...
      {myvehicledata.map((item, index) => (     
        ...
        >
          ...
          <div>
              ...
              {/* <h1 className='card transform uppercase text-center'>{datetoepoch(item.date)}</h1>         */}
              {/* <button onClick={PayMonthly(0.0083333333333333,item.policyId)}>pay monthly fee</button>         */}
           </div>
